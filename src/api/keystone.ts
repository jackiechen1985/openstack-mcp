...
export class KeystoneApi implements IKeystoneApi {
     ...
     * @returns 找到的 URL
     */
    getServiceUrlV2(catalog: AuthV2Response['access']['serviceCatalog'], type: string, regionName: string): string {
    ...
    async authV2(authUrl: string, username: string, password: string, projectName: string,
        ...
        try {
            ...
            const neutronUrl = this.getServiceUrlV2(serviceCatalog, 'network', regionName);
            const novaUrl = this.getServiceUrlV2(serviceCatalog, 'compute', regionName);
            const glanceUrl = this.getServiceUrlV2(serviceCatalog, 'image', regionName);
     ...
     * @returns 找到的 URL
     */
    getServiceUrlV3(catalog: AuthV3Response['token']['catalog'], type: string, regionName: string): string {
        const service = catalog.find(s => s.type === type);
        if (!service) {
            throw new Error(`未找到服务 (type: ${type})`);
        }
    ...
    async authV3(
        ...
        try {
            ...
            // 从 v3 服务目录中获取服务 URL
            const neutronUrl = this.getServiceUrlV3(catalog, 'network', regionName);
            const novaUrl = this.getServiceUrlV3(catalog, 'compute', regionName);
            const glanceUrl = this.getServiceUrlV3(catalog, 'image', regionName);
